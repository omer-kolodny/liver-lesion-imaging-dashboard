...
function classification(row) {
  if (row.kind === 'node') return { key:'node', cls:'node', label:'Extrahepatic node', note:'This portocaval nodal target is tracked separately and excluded from liver-lesion burden.' };
...
function renderOverview() {
  ...
  const relevant = hepatic.filter(row => measurement(row, fromDate).detected || measurement(row, toDate).detected);
  ...
  grid.innerHTML = visible.map(row => {
    ...
    return `<article class="lesion-card" tabindex="0" data-id="${row.lesion_id}" aria-label="Open ${row.lesion_id} details">
      ...
      <div class="lesion-body">
        <div class="lesion-top"><h3>${row.lesion_id} · ${row.kind === 'node' ? row.reference_label : `Segment ${row.reference_segment}`}</h3><span class="status ${status.cls}">${status.label}</span></div>
...
function openLesion(id) {
  ...
  dialogContent.innerHTML = `<div class="dialog-inner">
    <div class="dialog-header"><div><div class="eyebrow">${row.kind === 'node' ? 'Extrahepatic target timeline' : 'Lesion timeline'}</div><h2>${row.lesion_id} · ${row.kind === 'node' ? row.reference_label : `Reference segment ${row.reference_segment}`}</h2></div><span class="status ${status.cls}">${status.label}</span></div>
